<aside>
    <nav class="sidenav">
        <div class="nav-tabs">
            <a href="dashboard-client.php" class="tab">
                <i class="fa-solid fa-house"></i> Dashboard
            </a>
            <a href="dblist.php" class="tab">
                <i class="fa-solid fa-bars-staggered"></i> DB List
            </a>
            <a href="quality-checklist.php" class="tab">
                <i class="fa-solid fa-square-check"></i> Quality Checklist
            </a>
            <a href="add-website.php" class="tab">
                <i class="fa-solid fa-square-plus"></i> Add Website
            </a>
            <a href="uploads.php" class="tab">
                <i class="fa-solid fa-user"></i> Uploads
            </a>
        </div>
    </nav>
</aside>
